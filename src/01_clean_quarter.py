...
df = pd.read_csv("data/raw/Origin_and_Destination_Survey_DB1BMarket_2008_1.csv", usecols=cfg.DATA_COLS, 
                                                                                 dtype=cfg.DATA_TYPES)
df.info()
...
audit.append({"Filename": 'CHANGE', "Year": year, "Quarter": quarter, 
              "Raw Rows": raw_rows, "Raw Passengers": raw_pax})

audit.append({"Filename": 'CHANGE', "Year": year, "Quarter": quarter, "Filter": "File Read", "Rows Before": raw_rows, "Rows After": 
...
audit.append({"Filename": 'CHANGE', "Year": year, "Quarter": quarter, "Filter": "NaN Drop", "Rows Before": raw_rows, "Rows After": 
...
audit.append({"Filename": 'CHANGE', "Year": year, "Quarter": quarter, "Filter": "Bulk Fare", "Rows Before": post_nan_rows, "Rows After": 
...
audit.append({"Filename": 'CHANGE', "Year": year, "Quarter": quarter, "Filter": "Carrier Change", "Rows Before": post_bulk_rows, "Rows After": 
...
audit.append({"Filename": 'CHANGE', "Year": year, "Quarter": quarter, "Filter": "Geography Restriction", "Rows Before": post_cchange_rows, "Rows After": 
...
if cfg.TRIM_MODE == "fixed":
    ...
    audit.append({"Filename": 'CHANGE', "Year": year, "Quarter": quarter, "Filter": "Fare Min (fixed)", "Rows Before": post_mktgeo_rows, "Rows After": 
    ...
    audit.append({"Filename": 'CHANGE', "Year": year, "Quarter": quarter, "Filter": "Fare Max (fixed)", "Rows Before": post_faremin_rows, "Rows After": 
    ...
    post_faremin_rows = len(df)
    post_faremin_pax = df["Passengers"].sum()
    audit.append({"Filename": 'CHANGE', "Year": year, "Quarter": quarter, "Filter": "Fare Min (percentile)", "Rows Before": post_mktgeo_rows, "Rows After": 
              post_faremin_rows, "Passengers Before": post_mktgeo_pax, "Passengers After": post_faremin_pax})

    df = df[(df["MktFare"] < upper_limit)]
    post_faremax_rows = len(df)
    post_faremax_pax = df["Passengers"].sum()
    audit.append({"Filename": 'CHANGE', "Year": year, "Quarter": quarter, "Filter": "Fare Max (percentile)", "Rows Before": post_faremin_rows, "Rows After": 
              post_faremax_rows, "Passengers Before": post_faremin_pax, "Passengers After": post_faremax_pax})
...
assert np.isclose(g.pax.sum(), df.fare_x_pax.sum())

post_collapse_rows = len(g)
post_collapse_pax = g["pax"].sum()
audit.append({"Filename": 'CHANGE', "Year": year, "Quarter": quarter, "Filter": "Collapse", "Rows Before": post_faremax_rows, "Rows After": 
              post_collapse_rows, "Passengers Before": post_faremax_pax, "Passengers After": post_collapse_pax})

parquet_path = csv_folder = main_dir / "data" / "clean" / f"test_output.parquet"
